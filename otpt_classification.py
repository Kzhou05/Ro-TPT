...
def generate_next_orthogonal_matrix(q_previous, scale, device):
    """
    基于前一个正交矩阵 Q_{i-1}，生成下一个差异化的正交矩阵 Q_i。

    :param q_previous: 上一个正交矩阵 (Tensor).
    :param scale: 缩放因子，控制旋转幅度.
    :param device: 计算设备 (e.g., 'cuda' or 'cpu').
    :return: 新的正交矩阵 (Tensor).
    """
    # 确保在前一个矩阵所在的设备上进行计算
    dim = q_previous.shape[0]
    
    # # 1. 生成随机矩阵 A
    # a = torch.randn(dim, dim, device=device, dtype=q_previous.dtype)
    # # 2. 构造斜对称矩阵 S = A - A^T
    # s = a - a.T
    # # 3. 计算旋转矩阵 R = exp(scale * S)
    # r = matrix_exp(scale * s)

    # 3.1. 使用 torch.randn 生成一个随机矩阵
    normal_matrix = torch.randn(dim, dim, device=device)
    # 3.2. 进行 QR 分解
    r, _ = torch.linalg.qr(normal_matrix)
    identity = torch.eye(dim).to(device)
    r = scale * r + (1 - scale) * identity
    # 4. 施加旋转得到下一个正交矩阵 Q_i = R * Q_{i-1}
    q_next = r @ q_previous
...
def test_time_tuning(model, inputs, optimizer, scaler, args, cons):
    ...
    for j in range(args.tta_steps):
        ...
        if 'otpt' in args.run_type:
            ...
            # print("shapes:",transformed_matrix.shape,Wwt_exp.shape)
...
def main_worker(gpu, args):
    ...
    if args.cocoop:
        ...
        model = get_coop(args.arch, args.test_sets, args.gpu, args.n_ctx,args.ctx_init,args.disp_cons,args.alpha_term)
    ...
    for set_id in datasets:
        ...
        if args.tpt:
            ...
            if args.I_augmix:
                data_transform = AugMixAugmenter(base_transform, preprocess, n_views=args.batch_size-1, augmix=len(set_id)>=1)
            else:
                data_transform = AugMixAugmenter(base_transform, preprocess, n_views=args.batch_size-1, augmix=len(set_id)>1)
            batchsize = 1
        else:
    ...
    directory = os.path.dirname('./log/test_ctpt_finegrained_HT_-lambda-{}.csv'.format(len(dataset_ids)))
    custom_path = './log/test_ctpt_finegrained_HT_-lambda-{}.csv'.format(len(dataset_ids))
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, cons,classnames,set_id):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        # 设置参数
        feature_dim = 512  # 文本特征的维度，通常是512
        rotation_scale = 0.1 # 旋转幅度因子，可以调整
        device = image.device # 自动获取设备

        # 1. 初始化第一个随机正交矩阵
        torch.manual_seed(args.seed)
        # q_current = torch.linalg.qr(torch.randn(feature_dim, feature_dim, device=device))[0]
        q_current = model.q if hasattr(model, 'q') else torch.linalg.qr(torch.randn(feature_dim, feature_dim, device=device))[0]

        best_output = model(image, cons, args, rotation_matrix=q_current) #initial output
        best_l2_norm_mean = model.l2_norm_mean if hasattr(model, 'l2_norm_mean') else 0

        with torch.no_grad():
            with torch.cuda.amp.autocast():
                # --- MODIFIED LOOP ---
                for i in range(1):
                # 2. 在每次迭代中，生成下一个序贯旋转矩阵
                # 我们从上一个矩阵 q_current 生成新的，并更新它
                    q_current = generate_next_orthogonal_matrix(q_current, rotation_scale, device)

                    # 3. 将生成的矩阵作为参数传递给模型
                    if args.cocoop:
                        temp_output = model((image_feature, pgen_ctx), cons, args, rotation_matrix=q_current)
                    else:
                        temp_output = model(image, cons, args, rotation_matrix=q_current)
            
                    # Get l2_norm_mean from model 
                    l2_norm_mean = model.l2_norm_mean if hasattr(model, 'l2_norm_mean') else 0
            
                    # Select the best output based on l2_norm_mean 
                    if l2_norm_mean > best_l2_norm_mean:
                        best_l2_norm_mean = l2_norm_mean
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tta_steps', default=2, type=int, help='test-time-adapt steps')
    ...
    parser.add_argument('--alpha_term' , type=float, default=0.0, help='alpha for c-tpt')
