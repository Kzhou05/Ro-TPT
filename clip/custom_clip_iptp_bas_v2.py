...
class ClipTestTimeTuning(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
        ...
        r_alpha, _ = torch.linalg.qr(torch.randn(dim, dim))
        identity = torch.eye(dim)
        r_alpha = self.alpha * r_alpha + (1 - self.alpha) * identity

        self.r_alpha = nn.Parameter(torch.Tensor(r_alpha).half().to(device))
    ...
    def inference(self, image,cons,args):
        ...
        text_features = text_features @ self.r_alpha
